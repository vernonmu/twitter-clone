...
## Part One
v * Fork the repo
v * Get familiar with the html/css
v * Right before your closing body tag, link to your jQuery found in the js folder.
v * Create a animations.js file and then link to it right before your the closing body tag
v * Use jQuery to add the following features:
  v * Initially, the Tweet button and the character count button should be hidden (CSS).
  v* When the user clicks on the textarea, the textarea should double in size and the character count and Tweet buttons should be revealed.
  v* As the user types, the character count should decrease.
  v * When there are 10 or less characters, the character counter should turn red.
  v * If the user puts in more than 140 characters, the tweet button should be disabled (and re-enabled when there are <= 140 chars).
  v * When the user successfully inputs characters and clicks the “Tweet” button, a new tweet should be created and added to the tweet stream in the main column, using the user’s fake profile image in the top left and username/fullname.

## Part Two
v * The tweet actions (Reply, Retweet, etc) should only show up when you hover over that individual tweet. Otherwise, they should be hidden.
v * The Retweets/timestamp/Reply areas should also be hidden by default. These should only expand if you click on the tweet. Have the students use a jQuery animation to accomplish the reveal, similar to how it’s done on Twitter.com
